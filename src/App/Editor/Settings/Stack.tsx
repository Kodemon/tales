...
export const StackSettings: React.SFC<{
  ...
  return (
    <Components key={`stack-${stack.id}`} style={{ padding: 10 }}>
      Components
      <div>
        <button type="button" onClick={() => addText(stack)}>
          <i className="fa fa-font" /> <span>Text</span>
        </button>
        <button type="button" onClick={() => addImage(stack)}>
...
/**
 * Add a image component to the stack.
 *
 * @param stack
 */
function addImage(stack: Stack) {
  stack.addComponent(
    {
      type: "image",
      settings: {
...
/**
 * Add a text component to the stack.
 *
 * @param stack
 */
function addText(stack: Stack) {
  stack.addComponent(
    {
      type: "text",
      style: {
